...
def parse_compressed_cslc_file_name(native_id):
    ...
    """
    dataset_json = datasets_json_util.DatasetsJson()
    ccslc_granule_regex = dataset_json.get(
        "L2_CSLC_S1_COMPRESSED")["match_pattern"]
    match_product_id = re.match(ccslc_granule_regex, native_id)

    if not match_product_id:
        raise ValueError(
            f"Compressed CSLC native ID {native_id} could not be parsed with regex from datasets.json"
        )
