...
class CslcCmrQuery(CmrQuery):
    ...
    def validate_args(self):
        ...
        if self.proc_mode == "reprocessing":
            if (
                self.args.native_id is None
                and self.args.start_date is None
    ...
    def extend_additional_records(
        self, granules, no_duplicate=False, force_frame_id=None
    ):
        ...
        for granule in granules:
            ...
            burst_id, acquisition_dts, acquisition_cycles, frame_ids = (
                parse_cslc_native_id(
                    granule_id, self.burst_to_frames, self.disp_burst_map_hist
                )
            )
            ...
            granule["frame_id"] = (
                frame_ids[0] if force_frame_id is None else force_frame_id
            )
...
since the first CSLC file for the batch was ingested which is greater than the grace period of {self.grace_mins} minutes")
    ...
    def retrieve_k_granules(
        ...
        while k_satified < k_minus_one:
            ...
            if end_date_object < datetime.strptime(
                EARLIEST_POSSIBLE_CSLC_DATE, CMR_TIME_FORMAT
            ):
                raise AssertionError(
    ...
    def query_cmr_by_native_id(self, args, token, cmr, settings, now, native_id):
        ...
        start_date = (acquisition_time - timedelta(minutes=15)).strftime(
            CMR_TIME_FORMAT
        )
    ...
    def query_cmr_by_frame_and_dates(
        ...
        new_granules = asyncio.run(
            async_query_cmr(new_args, token, cmr, settings, timerange, now, silent)
        )
        self.extend_additional_records(
    ...
    def query_cmr(self, args, token, cmr, settings, timerange, now):
        ...
        if self.proc_mode == "historical":
            ...
            all_granules = [
                ...
                if granule["acquisition_cycle"]
                in self.disp_burst_map_hist[frame_id].sensing_datetime_days_index
            ]
        ...
        # native_id search takes precedence over date range if both are specified
            ...
            # 2) Process each granule as if they were passed in as native_id
                ...
                if self.args.frame_id is not None:
                    ...
                    granules = asyncio.run(
                        async_query_cmr(args, token, cmr, settings, timerange, now)
                    )
